...
## Core Architecture
- **Tooling Engine**: `src/nornir_mcp/services/runner.py:execute()` is mandatory for all network tasks.
- **Inventory/Filter**: Use `src/nornir_mcp/services/inventory.py:get_filtered_nornir()` for inventory access. Reloads `config.yaml` from disk on EVERY tool call; do not cache `Nornir` instances.
- **NAPALM**: Route getter requests through `src/nornir_mcp/services/napalm.py:run_napalm_get()`.
