...
def test_validate_command_rejects_dangerous_pattern() -> None:
    assert validate_command("show version ; reload") == "Command contains disallowed pattern: ';'"


def test_validate_command_rejects_blacklisted_keyword() -> None:
    assert (
        validate_command("delete vlan 10") == "Command starts with a blacklisted keyword: 'delete'"
    )

    assert validate_command("reload") == "Command starts with a blacklisted keyword: 'reload'"
